...
import { TopicFilter } from '../hooks/useSessionCalendar';

type EventCalendarSessionProps = {
...
export const EventCalendarSession = ({
  session,
  topic,
  instructorId,
  handleBookCourseSession,
}: EventCalendarSessionProps) => {
  ...
  const instructor = instructorId
    ? session.instructors?.find((instructor) => instructor.id === instructorId)
    : session?.instructors?.[0];

  const priorityTopic =
    topic !== null && topic?.type === 'topic'
      ? session.courseTopics.find(
          (courseTopic) => courseTopic.id === topic.value,
        )
      : session.courseTopics?.[0];
  ...
  const language = session.language === 'en' && (
    <div className="flex items-center justify-end space-x-1 text-caption font-semibold text-gray-650">
      <SoundUp />
      <span>EN</span>
    </div>
  );
  ...
  const certificate = session.hasCertificate ? (
    <div className="flex flex-row items-center text-gray-650">
    ...
    <div className="rounded-3xl border border-gray-200 bg-white p-6">
      {lg ? (
        <>
          <div className="flex space-x-4">
            ...
            <div className="flex-1 space-y-4">
              ...
              <div className="flex divide-x divide-gray-650">
                {learningWay}
                {certificate}
              </div>
            </div>
        ...
        <>
          ...
          <div className="mt-3">{instructorLabel}</div>
        </>
      )}
