...
function LearningTrackListSidebar() {
  ...
  return (
    <div className="hidden border-l border-r border-gray-200 bg-gray-100 p-3 text-caption font-semibold lg:flex lg:flex-col">
      {!isLoading && (
        <>
          <MenuItem
            name={t('dashboardLearningTrackListPage.allTopics')}
            isCurrent={showAllTopics}
          />
          {catalogMenu?.learningWays?.length > 0 && (
            <MenuList
              legend={t('dashboardLearningTrackListPage.4LineLearning')}
          ...
          {catalogMenu?.topics?.length > 0 && (
            <MenuList
              legend={t('dashboardLearningTrackListPage.browseByTopic')}
...
export default LearningTrackListSidebar;
