...
import {
  ...
  ICourseDirectAccess,
} from '../../models/courseDirectAccess';

interface IAddCourseDirectAccessModal {
  isOpen: boolean;
  toggle: (bool?) => void;
  onSubmit: (arg: any, id?: string) => Promise<void>;
  headingText: string;
  currentEditableItem?: ICourseDirectAccess;
  onCloseUpdateModal?: () => void;
}

const AddCourseDirectAccessModal: FC<IAddCourseDirectAccessModal> = ({
  isOpen,
  toggle,
  onSubmit,
  headingText,
  ...
  useEffect(() => {
    if (currentEditableItem && currentEditableItem.id) {
      formik.setValues({
        ...
        courseId: currentEditableItem.course.id,
  ...
  const { getOptions: getCourseOptions, inputValue: courseInputValue } =
    useAsyncInput({
      ...
      apiParams: {
        id: formik.values.courseId || undefined,
      },
    });
    ...
    >
      ...
      <div className="flex flex-col space-y-4 pt-4">
        <form onSubmit={formik.handleSubmit} autoComplete="off">
            ...
            <div className="mb-3">
              ...
              >
                <RadioButton
                  ...
                  value={CourseDirectAccessorType.User}
                  onBlur={formik.handleBlur}
                  onChange={onRadioOptionChange}
                  checked={
                    formik.values.accessorType === CourseDirectAccessorType.User
                  }
                  readOnly
                />
              ...
              >
                <RadioButton
                  name="accessorType"
                  inputClassName={'p-3 cursor-pointer'}
                  value={CourseDirectAccessorType.Group}
                  onBlur={formik.handleBlur}
                  onChange={onRadioOptionChange}
                  checked={
              ...
              >
                <RadioButton
                  name="accessorType"
                  inputClassName={'p-3 cursor-pointer'}
                  value={CourseDirectAccessorType.Organization}
                  onBlur={formik.handleBlur}
                  onChange={onRadioOptionChange}
                  checked={
                    formik.values.accessorType ===
          ...
          {formik.values.accessorType === CourseDirectAccessorType.Group && (
            <InputSelect
              name="accessorId"
              formik={formik}
              isDisabled={!formik.values.accessorType}
          ...
          <InputSelect
            ...
            label={'Course *'}
            value={{
              label: courseInputValue.label || 'Please select',
...
export default AddCourseDirectAccessModal;
