...
interface IPreBookCourseSessionModal {
  isOpen: boolean;
  onClose: () => void;
...
export default function PreBookCourseSessionModal({
  courseOutline,
  isOpen,
  toggle,
  onClose,
}: IPreBookCourseSessionModal) {
    ...
    >
      <Dialog.Title as="div" className="flex justify-between">
        <span className="text-subheading font-semibold text-gray-900">
          {t('courseSessionsPage.preBookCourseSessionModal.title')}
        </span>
        <Close
      ...
      <div className="mt-4 mb-4 lg:mb-6">
        {t('courseSessionsPage.preBookCourseSessionModal.suggestionText')}
      </div>
      ...
      <div className="flex flex-col-reverse justify-end lg:flex-row">
        ...
        >
          {t('courseSessionsPage.preBookCourseSessionModal.stayOnPage')}
        </Button>
        <Link
        ...
        >
          <a tabIndex={-1}>
            <Button
              avoidFullWidth
              ref={buttonRef}
              className="mb-3 w-full font-semibold lg:ml-2 lg:mb-0"
              size="medium"
              variant="primary"
              type="button"
              iconRight={
            ...
            >
              {t('courseSessionsPage.preBookCourseSessionModal.takeMeThere')}
            </Button>
          </a>
        </Link>
      </div>
